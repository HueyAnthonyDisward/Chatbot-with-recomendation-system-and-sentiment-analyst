...
try:
    ...
    data = pd.read_csv(BASE_DIR / 'cleaned_amazon_reviews.csv')
...
try:
    bitext_data = pd.read_csv(DATA_DIR / 'bitext_cleaned.csv')
    intent_classifier = joblib.load(DATA_DIR / 'intent_classifier.pkl')
except FileNotFoundError as e:
...
try:
    with open(DATA_DIR / 'tfidf_vectorizer_user.pkl', 'rb') as f:
        sentiment_tfidf = pickle.load(f)
    with open(DATA_DIR / 'sentiment_model_user.pkl', 'rb') as f:
